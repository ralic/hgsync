...
IS_BARE=$(git rev-parse --is-bare-repository) \
    || (echo "Could not find git repo" ; return 1)
if test "z$IS_BARE" != ztrue; then
   # This is not a bare repo, cd to the toplevel
   TOPLEVEL=$(git rev-parse --show-toplevel) \
       || (echo "Could not find git repo toplevel" ; return 1)
   cd $TOPLEVEL || return 1
fi
GIT_DIR=$(git rev-parse --git-dir) || (echo "Could not find git repo" ; return 1)
